...
const ToolCard: React.FC<ToolCardProps> = ({ tool, onSelect, onFavorite, isFavorited, currentLang }) => {
  const t = translations[currentLang].tool;
  
  // 综合得分算法: (评分*10) + (评论/100) + (收藏/500) + (点击/1000)
      ...
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-5">
           <div className="flex gap-4 text-white text-[10px] font-bold">
              <span className="flex items-center gap-1"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>{tool.clickCount} {t.clicks}</span>
              <span className="flex items-center gap-1"><svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>{tool.favoritesCount} {t.favs}</span>
           </div>
        </div>
        <button 
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-3">
          <div>
            ...
            <span className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">{tool.category}</span>
          </div>
          <div className="flex flex-col items-end">
            ...
            <span className="text-[9px] font-bold text-slate-400">{tool.reviewCount} {t.reviews}</span>
          </div>
        </div>
        ...
        <div className="pt-5 border-t border-slate-50 flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-[9px] uppercase font-bold text-slate-400 tracking-tighter">{t.score}</span>
          ...
          <a 
            ...
            onClick={(e) => { e.stopPropagation(); /* parent click logic would trigger click count in real app */ }}
